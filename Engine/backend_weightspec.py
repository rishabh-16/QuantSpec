import torch
from QuantSpec_magidec.Engine.model_weightspec import Transformer
from QuantSpec_magidec.Engine.utils import load_model_weightspec
...
class LMBackend:
    ...
    def load_model(self, checkpoints: str, use_tp: bool, rank_group=None, group = None, quantize: bool = False, marlin_checkpoint: str = None):
        ...
        self.model: Transformer = load_model_weightspec(checkpoint_path=checkpoints, device=self.device, precision=self.dtype, use_tp= use_tp, rank_group=rank_group, group = group, quantize = quantize, marlin_checkpoint = marlin_checkpoint)
    ...
    def draft_inference(self, input_ids: torch.LongTensor, benchmark = False, cachelen_update = None):
            ...
            logits = self.draft_forward[dec_len](
                ...
                cache_seqlens=self.cachelens) if dec_len in self.draft_forward.keys() else self.model.draft_forward(input_ids.clone(), position_ids.clone(), self.cachelens.clone())
            if not benchmark:
