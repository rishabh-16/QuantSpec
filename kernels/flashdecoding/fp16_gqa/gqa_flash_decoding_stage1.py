...
@triton.jit
def _fwd_kernel_gqa_flash_decode_stage1(
    Q, K, V, sm_scale,
    ...
    off_kbh = cur_batch * stride_kbs + cur_kv_head * stride_kh
    off_vbh = cur_batch * stride_vbs + cur_kv_head * stride_vh
    ...
    for start_n in range(0, block_n_size, 1):
        ...
        att_value = tl.sum(q[:, None, :] * k[None, :, :], 2)
        att_value *= sm_scale

        # tl.static_print(q, k, q[:, None, :] * k[None, :, :], att_value) # (4, 128), (64, 128), (4, 64, 128), (4, 64) 
        ...
        # tl.static_print(v, att_value, cur_max_logic, max_logic, new_max_logic) # (128, 64), (4, 64), 1, 1, 1

        exp_logic = tl.exp(att_value - new_max_logic[:, None])
        logic_scale = tl.exp(max_logic - new_max_logic)
        acc *= logic_scale[:, None]
        acc += tl.sum(exp_logic[:, None, :] * v[None, :, :], axis=2)

        # tl.static_print(exp_logic, logic_scale, acc) # (4, 64), 1, (4, 128)
...
@torch.no_grad()
def gqa_flash_decode_stage1(q, k, v, mid_out, mid_out_logsumexp, qcache_len, block_seq):
    """
    q : torch.Tensor
    ...
    sm_scale = 1.0 / (Lk ** 0.5)
    batch, kv_head_num = q.shape[0], k.shape[1]
    grid = (batch, kv_head_num, triton.cdiv(qcache_len, BLOCK_SEQ))
    gqa_group_size = q.shape[1] // k.shape[1]

    # import IPython
    # IPython.embed()

    _fwd_kernel_gqa_flash_decode_stage1[grid](
        q, k, v, sm_scale,
        mid_out,
        mid_out_logsumexp,
