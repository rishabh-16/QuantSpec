...
@triton.jit
def _fwd_kernel_int4kv_int8pack_flash_decode_stage1(
    Q, # query
    ...
    for start_n in range(0, block_n_size, 1): # Along the sequence length axis
        ...
        k_int8_low = (quant_k & 0xF)
        k_int8_high = ((quant_k & 0xF0) >> 4)
        k = tl.interleave(k_int8_low, k_int8_high)
        ...
        v_int8_low = (quant_v & 0xF)
        v_int8_high = ((quant_v & 0xF0) >> 4)
        v = tl.interleave(v_int8_low, v_int8_high).to(tl.float16)
...
@torch.no_grad()
def int4kv_int8pack_flash_decode_stage1(
    q,
    cache_quant_k, 
    cache_scale_k, 
    cache_min_k, 
    ...
    # shape constraints
    Lq, Lk = q.shape[-1], cache_quant_k.shape[-1] * elem_per_int
    assert Lq == Lk
    ...
    asm = _fwd_kernel_int4kv_int8pack_flash_decode_stage1[grid](
        q, 
