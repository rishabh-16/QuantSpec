...
@triton.jit
def _fwd_kernel_int4kv_int32pack_flash_decode_stage1(
    Q, # query
    Quant_K, Scale_K, Min_K,
    Quant_V, Scale_V, Min_V,
    sm_scale,
    ...
    offs_qk_d = tl.arange(0, BLOCK_DMODEL) # K do not quantize along the head_dim axis
    # Value use this to unpack int2/int4 values
    offs_quant_v_d = tl.arange(0, BLOCK_DMODEL) // elem_per_int # V quantize along the head_dim axis, so we need to divide it
    offs_scale_v_d = tl.arange(0, BLOCK_DMODEL // group_size) # V quantize along the head_dim axis, so we need to divide it
    v_shifter = (tl.arange(0, BLOCK_DMODEL) % elem_per_int) * vbit

    # Key use this
    cur_seq_start_index_quant = seq_start_block * BLOCK_SEQ_PER_INT
    cur_seq_start_index_scale = seq_start_block * BLOCK_SEQ // group_size
    # Value use this
    cur_seq_start_index = seq_start_block * BLOCK_SEQ

    # Query states offset
    off_q = cur_batch * stride_qbs + cur_head * stride_qh + offs_qk_d
    ...
    offs_n_quant = cur_seq_start_index_quant + tl.arange(0, BLOCK_N) // elem_per_int
    offs_n_scale = cur_seq_start_index_scale + tl.arange(0, BLOCK_N // group_size)
    k_shifter = (tl.arange(0, BLOCK_N) % elem_per_int) * kbit
    # Value use this
    ...
    for start_n in range(0, block_n_size, 1): # Along the sequence length axis
        offs_n_quant_k_new = start_n * BLOCK_N_PER_INT + offs_n_quant
        offs_n_scale_k_new = (start_n * BLOCK_N) // group_size + offs_n_scale

        off_quant_k = off_quant_kbh + offs_n_quant_k_new[:, None] * stride_quant_ks + offs_qk_d[None, :]
        off_scale_k = off_scale_kbh + offs_n_scale_k_new[:, None] * stride_scale_ks + offs_qk_d[None, :] # scale and zeropoint share this offset
        quant_k = tl.load(Quant_K + off_quant_k)
        ...
        if kbit == 4:
            ...
            k = (quant_k >> k_shifter[:, None]) & 0x3
        elif kbit == 8:
            k = (quant_k >> k_shifter[:, None]) & 0xFF
        else:
            tl.static_assert(False, "kbit is not 2 or 4")
        ...
        offs_n_v_new = start_n * BLOCK_N + offs_n

        off_quant_v = off_quant_vbh + offs_quant_v_d[:, None] * stride_quant_vd + offs_n_v_new[None, :]
        off_scale_v = off_scale_vbh + offs_scale_v_d[:, None] * stride_scale_vd + offs_n_v_new[None, :]
        quant_v = tl.load(Quant_V + off_quant_v)
        scale_v = tl.load(Scale_V + off_scale_v)
        min_v = tl.load(Min_V + off_scale_v)
        
        if vbit == 4:
            v = (quant_v >> v_shifter[:, None]) & 0xF
        elif vbit == 2:
            v = (quant_v >> v_shifter[:, None]) & 0x3
        elif vbit == 8:
            v = (quant_v >> v_shifter[:, None]) & 0xFF
        else:
            tl.static_assert(False, "vbit is not 2 or 4")
    ...
    off_mid_o = cur_batch * stride_mid_ob + cur_head * stride_mid_oh + seq_start_block * stride_mid_os + offs_qk_d # offset_d is renamed
...
@torch.no_grad()
def int4kv_int32pack_flash_decode_stage1(
    q,
    cache_quant_k, 
    cache_scale_k, 
    cache_min_k, 
    ...
    # shape constraints
    Lq, Lk = q.shape[-1], cache_quant_k.shape[-1]
    assert Lq == Lk
    ...
    asm = _fwd_kernel_int4kv_int32pack_flash_decode_stage1[grid](
        q, 
        cache_quant_k, cache_scale_k, cache_min_k, 
        cache_quant_v, cache_scale_v, cache_min_v,
        sm_scale,
        mid_out,
        mid_out_logsumexp,
        q.stride(0), q.stride(1), q.stride(2),
        cache_quant_k.stride(0), cache_quant_k.stride(1), cache_quant_k.stride(2),
        ...
        cache_quant_v.stride(0), cache_quant_v.stride(1), cache_quant_v.stride(2),
