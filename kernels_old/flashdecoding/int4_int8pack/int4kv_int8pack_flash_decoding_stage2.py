...
@triton.jit
def _fwd_kernel_int4kv_int8pack_flash_decode_stage2(
    ...
    stride_full_ob, stride_full_oh, stride_full_os, stride_full_od,
    stride_full_o_eb, stride_full_o_eh, stride_full_o_es,
...
@torch.no_grad()
def int4kv_int8pack_flash_decode_stage2(mid_out, mid_out_logexpsum, O, full_mid_out, full_logexpsum, max_len_in_batch, block_seq):
    Lk = mid_out.shape[-1]
    ...
    if full_mid_out is not None and full_logexpsum is not None:
        _fwd_kernel_int4kv_int8pack_flash_decode_stage2[grid](
