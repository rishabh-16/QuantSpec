...
@triton.jit
def _fwd_kernel_int8kv_int8pack_flash_decode_stage2(
...
@torch.no_grad()
def int8kv_int8pack_flash_decode_stage2(mid_out, mid_out_logexpsum, O, full_mid_out, full_logexpsum, max_len_in_batch, block_seq):
    Lk = mid_out.shape[-1]
    ...
    if full_mid_out is not None and full_logexpsum is not None:
        _fwd_kernel_int8kv_int8pack_flash_decode_stage2[grid](
