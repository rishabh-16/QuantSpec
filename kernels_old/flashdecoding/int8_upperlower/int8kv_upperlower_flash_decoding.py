...
from .int8kv_upperlower_flash_decoding_stage1 import int8kv_upperlower_flash_decode_stage1
from .int8kv_upperlower_flash_decoding_stage2 import int8kv_upperlower_flash_decode_stage2

def token_decode_attention_int8kv_upperlower_flash_decoding(
    q, \
    ...
    precision=8, cache_seqlens=None, qcache_seqlens=None,
):
    """
    q : torch.Tensor
    ...
    query_sequence_length = q.shape[2] # For Speculative Verification
    max_len_in_batch = cache_scale_v.shape[-1] if qcache_seqlens is None else torch.max(qcache_seqlens)

    elem_per_int = 8 // kbit # 8 since we use int8 to pack the quantized elements
    calcu_shape1 = (batch_size, q_head_num, query_sequence_length, head_dim)

    o_tensor = alloc_tensor_func(q.shape, dtype=q.dtype, device=q.device) if out is None else out

    mid_o = alloc_tensor_func(
        [batch_size, q_head_num, query_sequence_length, max_len_in_batch // BLOCK_SEQ + 1, head_dim], dtype=torch.float32, device=q.device
    )
    mid_o_logexpsum = alloc_tensor_func(
        [batch_size, q_head_num, query_sequence_length, max_len_in_batch // BLOCK_SEQ + 1], dtype=torch.float32, device=q.device
    )
    ...
    if full_k is not None and full_v is not None:
        ...
        attn_mask = torch.tril(torch.ones((query_sequence_length, full_k_seqlen), device=q.device), diagonal=full_k_seqlen-query_sequence_length)
    ...
    int8kv_upperlower_flash_decode_stage2(mid_o, mid_o_logexpsum, o_tensor.view(calcu_shape1), full_attn_weights, logExpSum, max_len_in_batch, BLOCK_SEQ)
