...
@triton.jit
def _fwd_kernel_int8kv_upperlower_flash_decode_stage2(
...
@torch.no_grad()
def int8kv_upperlower_flash_decode_stage2(mid_out, mid_out_logexpsum, O, full_mid_out, full_logexpsum, max_len_in_batch, block_seq):
    Lk = mid_out.shape[-1]
    assert Lk in {16, 32, 64, 128, 256, 512}
    batch, head_num = mid_out.shape[0], mid_out.shape[1]
    grid = (batch, head_num)

    q_seq_len = mid_out.shape[2]
